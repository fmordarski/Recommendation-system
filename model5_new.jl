...
for i in is
    ...
    print("  minimum k:   ")
    print(steps_initial[argmin(rmse_list_temp)])
    k = steps_initial[argmin(rmse_list_temp)]
    if k in steps
        push!(recommenders, i=>recommenders_temp[k])
    end
end
@save "rmse_list5_new.jld2" rmse_list
@save "model5_new.jld2" recommenders mappings train
